...
void Flooding::handleReceivedBroadcast(Packet* packet)
{
    ...
    ss << "handleReceivedBroadcast: got flooding packet with src = " << srcid
       << ", seqno = " << seqno
       << ", and ttl = " << ttl
       << ", map contents is " << found
       << (found ? (lastSeen[srcid]) : -1 );
    debugMsg(ss.str());
    ...
    if (     (srcid == ownIdentifier)
          || (found  &&  (lastSeen[srcid] >= seqno)))
    {
        debugMsg("handleReceivedBroadcast: This is my own or I have already seen this, dropping it and stop");
        delete packet;
